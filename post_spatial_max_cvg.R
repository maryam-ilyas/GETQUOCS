...
##HadCRUT4 median ensemble member
	...
	date_50<- format(TIME.t[986], "%B-%Y")     #"February-1932" 
	date_max<- format(TIME.t[1658], "%B-%Y")    #"February-1988"

##max coverage, February-1988
##data
Y<- c(tempr.a[,,id])
ind_s<- !is.na(Y)
y<- Y[ind_s]  
...
x<- X[ind_s,]
	...
	image.plot(mapping, col=lut, main= paste("Data (",date_50,"), n=",length(y) ), 
...
aw_abcc<- AA[,,1658][,2][-11]  	#11th value is tolerance threshold
lam_abcc<- AA[,,1658][,1][-11]	#11th value is tolerance threshold

###Approximate Bayesian Inference
###Consider likely values of lambda and a.wght 
theta_l_hat<- lam_abcc		#theta_l[ind]     #likely values of lambda
theta_aw_hat<- aw_abcc 		#theta_aw[ind]   #likely values of a.wght
...
yhat_old<- as.numeric(SS[id[1],])
	...
	X11()
	par(op) 		
	e<- paste("C:/Users/maryam/Dropbox/AMT_paper/spatial_estimate_abc_feb_1988_new_2.png")
      png(e,width = 13, height = 13, units = 'in', res = 300)
	par(oma=c(1,1,1,0),mar=c(7,3,3,2),mfrow=c(2,2))

	zr<- range(Y,yhat, na.rm=TRUE);zr  
	brks = seq(-12,12, by=0.8);brks ;length(brks) 	 #make sure the sequence has zero
...
##(a)-(b)
	...
	zr2<- range(yhat-yhat_old);zr2 
	brks2 = seq(-5.8,3.3, by=0.2);brks2 ;length(brks2) 	     #make sure the sequence has zero
...
SE.MIN<- as.numeric(SSS[id[1],])
	...
	X11()
	par(op) 		
	e<- paste("C:/Users/maryam/Dropbox/AMT_paper/spatial_estimate_abc_feb_1988_new_1.png")
  	png(e,width = 13, height = 13, units = 'in', res = 300)
	par(oma=c(1,1,1,0),mar=c(7,3,3,2),mfrow=c(2,2))

	zr1<- range(sd_yhat,SE.MIN);zr1
	brks1 =  seq(0.17,1.7,by=0.05);brks1 ;length(brks1)
...
##(a)-(b)
	...
	brks2 =  seq(-1,0.04,by=0.04);brks2 ;length(brks2)
	warm2 = rainbow(length(brks2)-1, start=rgb2hsv(col2rgb('red'))[1], end=rgb2hsv(col2rgb('yellow'))[1])
	lut2 = c(rev(warm2))
	NL2=length(lut2) 
